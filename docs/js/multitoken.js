...
window.addEventListener('load', async function() {
    ...
    const newAstraMultiTokenDeployer = async function () {
        const multitokenContract = new web3js.eth.Contract(multiTokenABI, account);
        const networkContract = new web3js.eth.Contract(multiTokenNetworkABI, $('#multiTokenNetworkAddress').val());
        ...
        const data = multitokenContract.methods['init(address[],uint256[],string,string,uint8)'](multitokenTokens, multitokenWeights, multitokenName, multitokenSymbol, 18).encodeABI();
        const preTx = networkContract.methods.deploy(index, data);

        sendTransaction(preTx, 0, networkContract.options.address);
    };

    const newAstraBasicMultiTokenDeployer = async function () {
        const basicMultitokenContract = new web3js.eth.Contract(basicMultiTokenABI, account);
        const networkContract = new web3js.eth.Contract(multiTokenNetworkABI, $('#multiTokenNetworkAddress').val());
        ...
        console.log('multitokenName = ' + multitokenName);
        console.log('multitokenSymbol = ' + multitokenSymbol);
        const data = basicMultitokenContract.methods['init(address[],string,string,uint8)'](multitokenTokens, multitokenName, multitokenSymbol, 18).encodeABI();
        const preTx = networkContract.methods.deploy(index, data);
    ...
    $('#new-multitoken-create').bind('click', async function () {
        ...
        if (title == 'AstraBasicMultiTokenDeployer') {
            await newAstraBasicMultiTokenDeployer();
        }
